...
using namespace Terrain;
        ...
        const SsrSettings* ssrSettings) const
    {
        ...
        const bool haveSceneColor = sceneColorCpu.ptr != 0 && depthCpu.ptr != 0;
        const bool canBindSsr     = pipeline.hasReceiverSrvs() && haveSceneColor;
        WaterPipeline::fillSsr(cb, camera, ssrSettings, !debug || debug->ssrEnabled, canBindSsr, env);
