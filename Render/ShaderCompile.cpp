...
bool compileShaderFromFile(const std::filesystem::path& path,
                           const char* entry,
                           const char* target,
                           ComPtr<ID3DBlob>& outBytecode)
{
    ...
    if (FAILED(hr))
    {
        ...
        DE_LOG_ERROR("Shader compile failed ({} {} from '{}'): {}",
                     entry,
                     target,
                     path.string(),
                     msg);
...
bool compileShaderFromContent(const char* relativeUnderContent,
                              const char* entry,
                              const char* target,
                              ComPtr<ID3DBlob>& outBytecode)
{
    ...
    {
        DE_LOG_ERROR(
            "compileShaderFromContent: could not find '{}' under content roots "
