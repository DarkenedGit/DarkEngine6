...
| **L21** | **Color space stays K9.** GGX runs in the current UNORM-ish working space. Physically based *units* and *shapes*; not a linear-sRGB migration. **Superseded (color space):** [DESIGN-color-management.md](./DESIGN-color-management.md) **C0**. π + candela retune is [DESIGN-ibl.md](./DESIGN-ibl.md) rev 2 **PR3** (before bind), not this RFC. | Coupling PBR to a look-breaking sRGB change would make this unshippable. Follow-up can keep `PbrLighting.hlsli` and only change inputs. |
...
**v1 directional and punctual diffuse is `diffuseCol * NdotL` (no 1/π), specular is the full GGX term without an extra π.** Document this as “engine units, Frostbite shape.” ~~When the sRGB/linear follow-up lands, add `1/π` and retune `Environment` / candela defaults together.~~ Color-management landed without π ([DESIGN-color-management.md](./DESIGN-color-management.md) **C14**); **[DESIGN-ibl.md](./DESIGN-ibl.md)** rev 2 adds `1/π` and retunes sun/ambient/candela in IBL-stack **PR3** (before bind), not when IBL lands as one PR.
