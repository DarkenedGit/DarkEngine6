...
bool ParticlePipeline::create(ID3D12Device* device, bool additive)
{
    ...
    if (FailedHr(D3D12SerializeRootSignature(&rsDesc, D3D_ROOT_SIGNATURE_VERSION_1, &rsBlob, &rsErr),
                 "Particle RS serialize"))
        return false;
    if (FailedHr(device->CreateRootSignature(0, rsBlob->GetBufferPointer(), rsBlob->GetBufferSize(),
                                             IID_PPV_ARGS(&m_rootSignature)),
                 "Particle CreateRootSignature"))
        return false;
