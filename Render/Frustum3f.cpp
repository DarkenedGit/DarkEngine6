...
namespace Dark
{
    namespace Math
    {
        ...
        void Frustum3f::Update(const Matrix4f& vp, bool normalize)
        {
            ...
            // Right
            Planes[Right] = Plane4f(vp(0, 3) - vp(0, 0), vp(1, 3) - vp(1, 0), vp(2, 3) - vp(2, 0), vp(3, 3) - vp(3, 0));

            // Top
            Planes[Top] = Plane4f(vp(0, 3) - vp(0, 1), vp(1, 3) - vp(1, 1), vp(2, 3) - vp(2, 1), vp(3, 3) - vp(3, 1));
        ...
        bool Frustum3f::Intersects(const Aabb3f& box) const
        {
            ...
            for (const auto& plane : Planes)
            {
                Vector3f p(plane.x >= 0.0f ? box.Max.x : box.Min.x, plane.y >= 0.0f ? box.Max.y : box.Min.y, plane.z >= 0.0f ? box.Max.z : box.Min.z);
        ...
        bool Frustum3f::Intersects(const Box3f& box) const
        {
            ...
            for (const auto& plane : Planes)
            {
                ...
                float    r = box.Extent[0] * fabsf(n.Dot(box.Axis[0])) + box.Extent[1] * fabsf(n.Dot(box.Axis[1])) + box.Extent[2] * fabsf(n.Dot(box.Axis[2]));
