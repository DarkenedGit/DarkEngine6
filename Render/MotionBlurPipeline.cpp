...
namespace Dark
{
    ...
    bool MotionBlurPipeline::create(ID3D12Device* device)
    {
        ...
        if (!device)
        {
            DE_LOG_ERROR(LogCategory::Render, "MotionBlurPipeline::create: null device");
            return false;
        }
        ...
        if (FailedHr(D3D12SerializeRootSignature(&rsDesc, D3D_ROOT_SIGNATURE_VERSION_1, &rsBlob, &rsErr), "D3D12SerializeRootSignature (motion blur)"))
        {
            if (rsErr)
                DE_LOG_ERROR(LogCategory::Render, "MotionBlur RS: {}", static_cast<const char*>(rsErr->GetBufferPointer()));
            return false;
        }
        if (FailedHr(device->CreateRootSignature(0, rsBlob->GetBufferPointer(), rsBlob->GetBufferSize(), IID_PPV_ARGS(&m_rootSignature)), "CreateRootSignature (motion blur)"))
            return false;

        ComPtr<ID3DBlob> vs;
        ComPtr<ID3DBlob> ps;
        if (!compileShaderFromContent("shaders/MotionBlur.hlsl", "VSMain", "vs_5_0", vs) || !compileShaderFromContent("shaders/MotionBlur.hlsl", "PSMain", "ps_5_0", ps))
            return false;
        ...
        pso.SampleDesc                                       = { 1, 0 };
        if (FailedHr(device->CreateGraphicsPipelineState(&pso, IID_PPV_ARGS(&m_pso)), "CreateGraphicsPipelineState (motion blur)"))
        {
            m_rootSignature.Reset();
            return false;
        }
        ...
        if (FailedHr(device->CreateDescriptorHeap(&heapDesc, IID_PPV_ARGS(&m_srvHeap)), "CreateDescriptorHeap motion blur SRV"))
        {
            m_pso.Reset();
            m_rootSignature.Reset();
            return false;
        ...
        DE_LOG_INFO(LogCategory::Render, "MotionBlurPipeline: ready");
        return true;
    }

    void MotionBlurPipeline::draw(ID3D12GraphicsCommandList* cmd, Renderer& renderer, const MotionBlurSettings& settings) const
    {
        if (!cmd || !m_pso || !m_srvHeap)
            return;
        ID3D12Device* device = renderer.device();
        const D3D12_CPU_DESCRIPTOR_HANDLE src = settings.readPost ? renderer.postHdrSrvCpu() : renderer.hdrSrvCpu();
        ...
        D3D12_CPU_DESCRIPTOR_HANDLE velCpu = cpu;
        ...
        constants[32] = settings.strength;
        constants[33] = settings.maxPixels;
        constants[34] = 1.0f / w;
        constants[35] = 1.0f / h;
