...
Vector3f ParticleEmitter::randomUnitDirectionInCone(const Vector3f& axisIn, float spreadRad)
{
    ...
    Vector3f tangent = normalizeSafe(
        Vector3f(axis.y * helper.z - axis.z * helper.y,
    ...
    return normalizeSafe(
        Vector3f(
            axis.x * cosTheta + tangent.x * (sinTheta * std::cos(phi)) + bitangent.x * (sinTheta * std::sin(phi)),
...
void ParticleEmitter::spawnOne()
{
    ...
    case ParticleEmitterDesc::Shape::Box:
        localPos = Vector3f(
            (rand01() * 2.0f - 1.0f) * m_desc.shapeSize.x,
            (rand01() * 2.0f - 1.0f) * m_desc.shapeSize.y,
            (rand01() * 2.0f - 1.0f) * m_desc.shapeSize.z);
        break;
