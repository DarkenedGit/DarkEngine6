...
public:
    ...
    bool addSharedTransition(const std::vector<HsmState*>& sources,
                             HsmEventId eventId,
                             HsmState* target,
                             HsmGuard guard = {},
                             HsmAction action = {},
    ...
    HsmState* currentLeaf() const { return m_leaf; }
    bool      isIn(const HsmState* state) const;
    // Active path root -> ... -> leaf
    const std::vector<HsmState*>& activePath() const { return m_activePath; }
