...
bool HunterBrain::start()
{
    return m_machine.start();
}

void HunterBrain::tick(float dt, bool seesOnLand, bool playerInWater)
{
...
HunterLeaf HunterBrain::leaf() const
{
    if (m_machine.isIn(&m_chase))
        return HunterLeaf::Chase;
    if (m_machine.isIn(&m_memory))
        return HunterLeaf::Memory;
    return HunterLeaf::Wander;
}
