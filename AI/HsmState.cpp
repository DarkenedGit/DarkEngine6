...
HsmState::HsmState(std::string_view name)
    : m_name(name)
{
}
...
HsmState& HsmState::setInitial(HsmState* child)
{
    ...
    {
        DE_LOG_ERROR("HsmState '{}': initial child '{}' is not a direct child",
                     m_name,
                     child ? child->m_name : std::string("(null)"));
        return *this;
    }
    m_initial = child;
    return *this;
...
HsmState& HsmState::addTransition(HsmEventId eventId,
                                  HsmState* target,
                                  HsmGuard guard,
                                  HsmAction action,
