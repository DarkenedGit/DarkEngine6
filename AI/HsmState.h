...
public:
    ...
    HsmState& addTransition(HsmEventId eventId,
                            HsmState* target,
                            HsmGuard guard = {},
                            HsmAction action = {},
    ...
    HsmState*          initial() const { return m_initial; }
    HsmState*          base() const { return m_base; }
    HsmHistory         history() const { return m_history; }
    bool               inheritEntryExit() const { return m_inheritEntryExit; }
    bool               isComposite() const { return !m_children.empty(); }
    bool               isAncestorOf(const HsmState* other) const;
    int                depth() const;

    const std::vector<HsmState*>&     children() const { return m_children; }
    const std::vector<HsmTransition>& transitions() const { return m_transitions; }
    const std::vector<HsmAction>&     enterActions() const { return m_enter; }
    const std::vector<HsmAction>&     exitActions() const { return m_exit; }
    const std::vector<HsmAction>&     eventHandlers() const { return m_onEvent; }
    const std::vector<HsmGuard>&      enterGuards() const { return m_enterGuards; }
    const std::vector<HsmGuard>&      exitGuards() const { return m_exitGuards; }
