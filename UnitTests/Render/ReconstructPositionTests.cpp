...
TEST(ReconstructPosition, NearAndFarOnRay)
{
    ...
    const Matrix4f          inv   = cam.GetViewProj().Inverse();
    const Vector3f          nearP = reconstruct(inv, 0.0f, 0.0f, 1.0f);
    const Vector3f          farP  = reconstruct(inv, 0.0f, 0.0f, 1.0e-6f);
    const Dark::Math::Ray3f ray   = cam.ScreenPointToRay(0.5f * 100.0f, 0.5f * 100.0f, 100.0f, 100.0f);

    const Vector3f originErr = ray.Origin - cam.GetPosition();
    EXPECT_LT(originErr.Magnitude(), 1.0e-3f);
    EXPECT_NEAR((nearP - ray.Origin).Magnitude(), cam.GetNearZ(), 1.0e-3f);

    Vector3f farDir = farP - ray.Origin;
    farDir.Normalize();
