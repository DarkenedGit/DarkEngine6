...
{
    if (stacks < 2) throw std::invalid_argument("stacks must be >= 2");
    if (slices < 3) throw std::invalid_argument("slices must be >= 3");
    MeshData m;
    ...
    for (int i = 0; i < stacks; ++i) {
        for (int j = 0; j < slices; ++j) {
            ...
            if (i != 0)        pushIdx(m, a, c, b);
            if (i != stacks-1) pushIdx(m, a, d, c);
        }
    }
    return m;
}
...
MeshData CreateCone(float radius, float height, int slices, bool capBase)
{
    ...
    if (capBase) {
        ...
        m.positions.push_back({0,-halfH,0}); m.normals.push_back({0,-1,0}); m.uvs.push_back({.5f,.5f});
        for (int i = 0; i <= slices; ++i) {
...
//  3. CYLINDER
// ============================================================
MeshData CreateCylinder(float radiusTop, float radiusBottom, float height,
                        int slices, bool capTop, bool capBottom)
{
    if (slices < 3) throw std::invalid_argument("slices must be >= 3");
    MeshData m;
    ...
    for (int i = 0; i <= slices; ++i) {
        ...
        m.normals  .push_back({nR*cosT, nY, nR*sinT}); m.uvs.push_back({t, 0.f});
        m.positions.push_back({radiusTop*cosT,    halfH, radiusTop*sinT});
        m.normals  .push_back({nR*cosT, nY, nR*sinT}); m.uvs.push_back({t, 1.f});
    }
    for (int i = 0; i < slices; ++i) {
    ...
    auto addCap = [&](float r, float y, float ny) {
        uint32_t base = (uint32_t)m.positions.size();
        m.positions.push_back({0,y,0}); m.normals.push_back({0,ny,0}); m.uvs.push_back({.5f,.5f});
        for (int i = 0; i <= slices; ++i) {
            float theta = (float)i/slices * twoPi;
            float cosT = cosf(theta), sinT = sinf(theta);
        ...
        for (int i = 0; i < slices; ++i) {
            if (ny > 0) pushIdx(m, base, base+1+i, base+2+i);
            else        pushIdx(m, base, base+2+i, base+1+i);
        }
    };
    if (capTop)    addCap(radiusTop,     halfH,  1.f);
    if (capBottom) addCap(radiusBottom, -halfH, -1.f);
    return m;
}
...
// ============================================================
static void addBoxFace(MeshData& m,
                       float3 p0, float3 p1, float3 p2, float3 p3, float3 n,
                       float2 uv0, float2 uv1, float2 uv2, float2 uv3)
{
    uint32_t b = (uint32_t)m.positions.size();
    float3 pts[4]  = {p0, p1, p2, p3};
    float2 uvs_[4] = {uv0, uv1, uv2, uv3};
    for (int i = 0; i < 4; ++i) {
...
//  4. CUBE
// ============================================================
MeshData CreateCube(float size) { return CreateCuboid(size, size, size); }
...
MeshData CreatePyramid(float baseSize, float height)
{
    ...
    float3 BL={-hb,-hh,-hb},BR={hb,-hh,-hb},FR={hb,-hh,hb},FL={-hb,-hh,hb},AP={0,hh,0};

    auto addTri = [&](float3 a, float3 b, float3 c) {
        float3 n = normalize(cross(sub(b,a),sub(c,a)));
        uint32_t base = (uint32_t)m.positions.size();
        m.positions.push_back(a); m.normals.push_back(n); m.uvs.push_back({.5f,1});
        m.positions.push_back(b); m.normals.push_back(n); m.uvs.push_back({0,0});
        m.positions.push_back(c); m.normals.push_back(n); m.uvs.push_back({1,0});
        pushIdx(m, base, base+1, base+2);
    };
    addTri(AP,FL,FR); addTri(AP,FR,BR); addTri(AP,BR,BL); addTri(AP,BL,FL);

    uint32_t base = (uint32_t)m.positions.size();
    float3 bn={0,-1,0};
    m.positions.push_back(BL); m.normals.push_back(bn); m.uvs.push_back({0,1});
    m.positions.push_back(BR); m.normals.push_back(bn); m.uvs.push_back({1,1});
    m.positions.push_back(FR); m.normals.push_back(bn); m.uvs.push_back({1,0});
    m.positions.push_back(FL); m.normals.push_back(bn); m.uvs.push_back({0,0});
...
MeshData CreateOctahedron(float radius)
{
    ...
    float3 V[6] = {{r,0,0},{-r,0,0},{0,r,0},{0,-r,0},{0,0,r},{0,0,-r}};
    int F[8][3] = {{2,0,4},{2,4,1},{2,1,5},{2,5,0},{3,4,0},{3,1,4},{3,5,1},{3,0,5}};
    for (auto& f : F) {
        float3 a=V[f[0]],b=V[f[1]],c=V[f[2]];
        float3 n=normalize(cross(sub(b,a),sub(c,a)));
        uint32_t base=(uint32_t)m.positions.size();
        m.positions.push_back(a); m.normals.push_back(n); m.uvs.push_back({.5f,1});
        m.positions.push_back(b); m.normals.push_back(n); m.uvs.push_back({0,0});
...
MeshData CreateTriangularPrism(float baseSize, float height)
{
    ...
    for (int i = 0; i < 3; ++i) {
        int j = (i+1)%3;
        float3 p[4] = {bot[i],bot[j],top_[j],top_[i]};
        float3 n = normalize(cross(sub(p[1],p[0]),sub(p[2],p[0])));
        uint32_t base=(uint32_t)m.positions.size();
        float2 uv[4]={{0,0},{1,0},{1,1},{0,1}};
        for (int k=0;k<4;++k){m.positions.push_back(p[k]);m.normals.push_back(n);m.uvs.push_back(uv[k]);}
        pushIdx(m,base+0,base+1,base+2);
        pushIdx(m,base+0,base+2,base+3);
    }
    auto triCap=[&](float3 a,float3 b,float3 c,float3 n){
        uint32_t base=(uint32_t)m.positions.size();
        m.positions.push_back(a);m.normals.push_back(n);m.uvs.push_back({.5f,1});
        m.positions.push_back(b);m.normals.push_back(n);m.uvs.push_back({0,0});
        m.positions.push_back(c);m.normals.push_back(n);m.uvs.push_back({1,0});
...
MeshData CreateDodecahedron(float radius)
{
    ...
    const float3 V[20] = {
        ...
        {-a, a, a},{-a, a,-a},{-a,-a, a},{-a,-a,-a},
        { 0.f, c_, b_},{ 0.f, c_,-b_},{ 0.f,-c_, b_},{ 0.f,-c_,-b_},
        { b_, 0.f, c_},{-b_, 0.f, c_},{ b_, 0.f,-c_},{-b_, 0.f,-c_},
        { c_, b_, 0.f},{ c_,-b_, 0.f},{-c_, b_, 0.f},{-c_,-b_, 0.f}
    };
    ...
    for (auto& face : F) {
        float3 cen={0,0,0};
        for (int k=0;k<5;++k){cen.x+=V[face[k]].x;cen.y+=V[face[k]].y;cen.z+=V[face[k]].z;}
        float3 fn = normalize(cen);
        float3 v0={V[face[0]].x*sc,V[face[0]].y*sc,V[face[0]].z*sc};
        for (int k=1;k<4;++k) {
            float3 vk ={V[face[k  ]].x*sc,V[face[k  ]].y*sc,V[face[k  ]].z*sc};
            float3 vk1={V[face[k+1]].x*sc,V[face[k+1]].y*sc,V[face[k+1]].z*sc};
            uint32_t base=(uint32_t)m.positions.size();
            m.positions.push_back(v0); m.normals.push_back(fn); m.uvs.push_back({.5f,1});
            m.positions.push_back(vk);m.normals.push_back(fn); m.uvs.push_back({0,0});
            m.positions.push_back(vk1);m.normals.push_back(fn);m.uvs.push_back({1,0});
            pushIdx(m,base,base+1,base+2);
        }
    }
    return m;
...
MeshData CreateHexagonalPrism(float radius, float height, bool capTop, bool capBottom)
{
    ...
    for (int i=0;i<N;++i) {
        ...
        float3 p[4]={{radius*c0,-hh,radius*s0},{radius*c1,-hh,radius*s1},
                     {radius*c1, hh,radius*s1},{radius*c0, hh,radius*s0}};
        float3 n=normalize(cross(sub(p[1],p[0]),sub(p[3],p[0])));
        uint32_t base=(uint32_t)m.positions.size();
        float2 uv[4]={{0,0},{1,0},{1,1},{0,1}};
        for (int k=0;k<4;++k){m.positions.push_back(p[k]);m.normals.push_back(n);m.uvs.push_back(uv[k]);}
        pushIdx(m,base+0,base+1,base+2);
    ...
    if(capTop)    addCap( hh, 1.f);
    if(capBottom) addCap(-hh,-1.f);
    return m;
}
...
MeshData CreateEllipsoid(float rx, float ry, float rz, int stacks, int slices)
{
    ...
    for (int i=0;i<=stacks;++i) {
        ...
        for (int j=0;j<=slices;++j) {
            ...
            float3 n=normalize({px/(rx*rx),py/(ry*ry),pz/(rz*rz)});
...
{
    if (majorSlices < 3) throw std::invalid_argument("majorSlices must be >= 3");
    if (minorSlices < 3) throw std::invalid_argument("minorSlices must be >= 3");
    MeshData m;
    const float twoPi=2.f*(float)M_PI;
    for (int i=0;i<=majorSlices;++i) {
        ...
        for (int j=0;j<=minorSlices;++j) {
            ...
            float3 n=normalize({cosT*cosPhi,sinT,cosT*sinPhi});
            m.positions.push_back({px,py,pz});
            m.normals  .push_back(n);
...
{
    if (slices    < 3) throw std::invalid_argument("slices must be >= 3");
    if (capStacks < 1) throw std::invalid_argument("capStacks must be >= 1");
    if (height < 2.f * radius) height = 2.f * radius; // degenerate → sphere
...
{
    if (subdivisions < 0) throw std::invalid_argument("subdivisions must be >= 0");
    ...
    // Build flat list of triangles as float3 (positions on unit sphere)
    ...
    for (int sub = 0; sub < subdivisions; ++sub) {
        std::vector<std::array<float3,3>> next;
        next.reserve(tris.size() * 4);
        for (auto& tri : tris) {
            // Midpoints projected onto unit sphere
            auto mid = [](float3 a, float3 b) -> float3 {
                return normalize({(a.x+b.x)*.5f,(a.y+b.y)*.5f,(a.z+b.z)*.5f});
            };
            float3 m01 = mid(tri[0], tri[1]);
            float3 m12 = mid(tri[1], tri[2]);
            float3 m20 = mid(tri[2], tri[0]);
    ...
    if (subdivisions == 0) {
        ...
        for (auto& tri : tris) {
            float3 a = {tri[0].x*radius,tri[0].y*radius,tri[0].z*radius};
            float3 b = {tri[1].x*radius,tri[1].y*radius,tri[1].z*radius};
            float3 c = {tri[2].x*radius,tri[2].y*radius,tri[2].z*radius};
            float3 n = normalize(cross(sub(b,a), sub(c,a)));
            uint32_t base = (uint32_t)m.positions.size();
            m.positions.push_back(a); m.normals.push_back(n); m.uvs.push_back({0.5f,1.0f});
            m.positions.push_back(b); m.normals.push_back(n); m.uvs.push_back({0.0f,0.0f});
            m.positions.push_back(c); m.normals.push_back(n); m.uvs.push_back({1.0f,0.0f});
            pushIdx(m, base, base+1, base+2);
        }
    } else {
        ...
        for (auto& tri : tris) {
            uint32_t base = (uint32_t)m.positions.size();
            for (int k = 0; k < 3; ++k) {
                float3 n  = tri[k]; // already unit length
                float3 p  = {n.x*radius, n.y*radius, n.z*radius};
                // Spherical UVs
...
// ============================================================
MeshData CreateSpring(float coilRadius, float tubeRadius,
                      float coils,     float height,
                      int   coilSlices, int tubeSlices)
{
    if (coilSlices < 4) throw std::invalid_argument("coilSlices must be >= 4");
    if (tubeSlices < 3) throw std::invalid_argument("tubeSlices must be >= 3");
    ...
    for (int i = 0; i <= totalSteps; ++i) {
        ...
        float3 pos = { coilRadius * cosT, y, coilRadius * sinT };

        // Tangent: dC/dθ = (-R sinθ, yPerRad, R cosθ), then normalise
        float3 tangent = normalize({ -coilRadius * sinT, yPerRad, coilRadius * cosT });

        // Normal: points inward toward axis (−radial)
        float3 radial = { cosT, 0.f, sinT };
        float3 normal = { -radial.x, -radial.y, -radial.z };
    ...
    auto emitRing = [&](const Frame& f, float u) {
        for (int j = 0; j <= tubeSlices; ++j) {
            ...
            float3 offset = {
                tubeRadius * (cosP * f.N.x + sinP * f.B.x),
                tubeRadius * (cosP * f.N.y + sinP * f.B.y),
                tubeRadius * (cosP * f.N.z + sinP * f.B.z)
            };
            float3 pos = { f.pos.x + offset.x, f.pos.y + offset.y, f.pos.z + offset.z };
            float3 nrm = normalize(offset);
...
// ============================================================
MeshData CreateArch(float innerRadius, float outerRadius,
                    float depth, float arcAngle, int slices)
{
    if (slices    < 2) throw std::invalid_argument("slices must be >= 2");
    if (outerRadius <= innerRadius) throw std::invalid_argument("outerRadius must be > innerRadius");
    ...
    // ── Outer curved face ──────────────────────────────────────
    {
        for (int i = 0; i <= slices; ++i) {
            ...
            float3 n = { cosT, sinT, 0.f };
            float3 pF = { outerRadius * cosT, outerRadius * sinT,  halfD };
            float3 pB = { outerRadius * cosT, outerRadius * sinT, -halfD };
            m.positions.push_back(pF); m.normals.push_back(n); m.uvs.push_back({ t, 1.f });
            m.positions.push_back(pB); m.normals.push_back(n); m.uvs.push_back({ t, 0.f });
        }
        int stride = 2;
        for (int i = 0; i < slices; ++i) {
    ...
    // ── Inner curved face ──────────────────────────────────────
    {
        ...
        for (int i = 0; i <= slices; ++i) {
            ...
            float3 n = { -cosT, -sinT, 0.f };
            float3 pF = { innerRadius * cosT, innerRadius * sinT,  halfD };
            float3 pB = { innerRadius * cosT, innerRadius * sinT, -halfD };
            m.positions.push_back(pF); m.normals.push_back(n); m.uvs.push_back({ t, 1.f });
            m.positions.push_back(pB); m.normals.push_back(n); m.uvs.push_back({ t, 0.f });
        }
        int stride = 2;
    ...
    auto addFlatFace = [&](float z, float3 n) {
        uint32_t base = (uint32_t)m.positions.size();
        ...
        for (int i = 0; i < slices; ++i) {
            ...
            if (n.z > 0) {   // front face — CCW from +Z
                pushIdx(m, o0, in0, in1);
                pushIdx(m, o0, in1, o1);
            } else {          // back face — CCW from -Z
    ...
    auto addCap = [&](float t, float3 n) {
        float theta = startAngle + arcAngle * t;
        float cosT  = cosf(theta), sinT = sinf(theta);
        float3 outer = { outerRadius * cosT, outerRadius * sinT, 0 };
        float3 inner = { innerRadius * cosT, innerRadius * sinT, 0 };

        uint32_t base = (uint32_t)m.positions.size();
        // 4 corners: outerFront, outerBack, innerBack, innerFront
        float3 pts[4] = {
            { outer.x, outer.y,  halfD },
            { outer.x, outer.y, -halfD },
            { inner.x, inner.y, -halfD },
            { inner.x, inner.y,  halfD }
        };
        float2 uvs_[4] = {{0,1},{1,1},{1,0},{0,0}};
        for (int k = 0; k < 4; ++k) {
    ...
    // Left cap normal: tangent to arc at t=0, pointing outward from arch
    {
        ...
        float3 n = normalize({ sinf(theta), -cosf(theta), 0.f });
        addCap(0.f, n);
    }
...
MeshData CreateGroundPlane(float size, float y, float uvScale)
{
    ...
    const float3 bl = {-h, y, -h};
    const float3 br = { h, y, -h};
    const float3 tr = { h, y,  h};
    const float3 tl = {-h, y,  h};
    const float3 n  = {0, 1, 0};
    const float u = uvScale;

    auto pushV = [&](float3 p, float2 uv) {
        m.positions.push_back(p);
        m.normals.push_back(n);
        m.uvs.push_back(uv);
    };
    ...
    pushV(tr, {u, 0});
    pushV(tl, {0, 0});
    // Outward +Y with FrontCounterClockwise (same convention as CreateCuboid faces).
    pushIdx(m, 0, 2, 1);
    pushIdx(m, 0, 3, 2);
    return m;
}
